...
function ServiceCard({ icon, name, price, items, featured = false }: {
  ...
  return (
    <div className={`relative rounded-[24px] p-8 lg:p-10 border overflow-hidden transition-all duration-500 hover:-translate-y-2 group ${featured ? "bg-navy text-cream border-transparent" : "bg-white border-navy/[0.06] hover:shadow-[0_24px_60px_rgba(31,36,51,0.1)]"}`}>
      ...
      <h3 className="font-fraunces text-[26px] lg:text-[28px] font-bold mb-2">{name}</h3>
...
function ReviewCard({ text, name, meta, initial }: { text: string; name: string; meta: string; initial: string }) {
  return (
    <div className="bg-white p-6 lg:p-8 rounded-[20px] border border-navy/[0.06]">
      ...
      <div className="flex items-center gap-3">
        <div className="w-11 h-11 rounded-full bg-green text-white flex items-center justify-center font-bold text-sm flex-shrink-0">{initial}</div>
        <div>
...
export default function HomePage() {
  return (
    <>
      ...
      {/* ── HERO ── */}
      <section className="relative min-h-screen grid grid-cols-1 md:[grid-template-columns:1.1fr_0.9fr] gap-8 md:gap-[60px] items-center pt-28 pb-12 px-4 sm:px-6 lg:px-12 md:pt-[140px] md:pb-20">
        ...
        <div className="relative z-[2]">
          <span className="inline-flex items-center gap-2 px-4 py-2 bg-white rounded-full text-[13px] font-semibold text-green-dark mb-6 border border-navy/[0.08]">
            <span className="w-2 h-2 rounded-full bg-green animate-pulse-dot" />
            Now serving Lake George &amp; Mt. Pleasant
          </span>
          <h1 className="display animate-fade-up mb-6" style={{ fontSize: "clamp(40px, 7vw, 88px)" }}>
            Local care<br />for every <em className="italic text-green-dark font-semibold">ride.</em>
          </h1>
          <p className="animate-fade-up text-warm-700 max-w-xl mb-8" style={{ fontSize: 18, lineHeight: 1.6, animationDelay: "0.1s" }}>
          ...
          <div className="animate-fade-up flex gap-3 flex-wrap mb-10" style={{ animationDelay: "0.2s" }}>
          ...
          {/* Stats */}
          <div className="animate-fade-up flex flex-wrap gap-8 pt-6 border-t border-navy/10" style={{ animationDelay: "0.3s" }}>
            {[{ num: "$69", label: "Starting price" }, { num: "9 to 8", label: "Open daily" }, { num: "★ 5.0", label: "Local rating" }].map((s) => (
              <div key={s.label}>
                <div className="font-fraunces text-3xl lg:text-4xl font-extrabold text-navy">{s.num}</div>
        ...
        {/* Mascot card */}
        <div className="relative z-[1] w-full aspect-square max-w-[360px] mx-auto md:max-w-[540px] md:ml-auto animate-fade-up" style={{ animationDelay: "0.3s" }}>
          {/* Float badges — hidden on mobile to reduce clutter */}
          <div className="hidden sm:flex animate-float absolute z-[3] bg-cream rounded-2xl items-center gap-3" style={{ top: -20, left: -30, padding: "16px 20px", boxShadow: "0 12px 32px rgba(31,36,51,0.12)" }}>
          ...
          <div className="hidden sm:flex animate-float-delay absolute z-[3] bg-cream rounded-2xl items-center gap-3" style={{ bottom: 40, right: -40, padding: "16px 20px", boxShadow: "0 12px 32px rgba(31,36,51,0.12)" }}>
            <div className="w-[42px] h-[42px] bg-green rounded-xl flex items-center justify-center text-white">
      ...
      {/* ── SERVICES ── */}
      <section id="services" className="py-16 lg:py-24 px-4 sm:px-6 lg:px-12">
        <div className="max-w-[1280px] mx-auto">
          <span className="section-label">Our Services</span>
          <h2 className="display mb-6" style={{ fontSize: "clamp(32px, 5vw, 60px)" }}>
            Three packages.<br />One promise: <em className="italic text-green-dark font-semibold">spotless.</em>
          </h2>
          <p className="text-lg text-warm-700 max-w-2xl mb-10 lg:mb-16">Pick what fits your car and your day. Every detail starts with a free quote, so there are no surprises and no upsells.</p>
      ...
      {/* ── WHY US ── */}
      <section id="why" className="bg-navy text-cream py-16 lg:py-24 px-4 sm:px-6 lg:px-12">
        <div className="max-w-[1280px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-[80px] items-center">
          <div>
            <span className="section-label text-green-light">Why GoHorn</span>
            <h2 className="display text-cream mb-8" style={{ fontSize: "clamp(32px, 5vw, 60px)" }}>
          ...
          <div className="flex flex-col gap-5">
            {[
              ...
              <div key={f.title} className="flex gap-5 p-5 lg:p-6 rounded-2xl border border-white/[0.06]" style={{ background: "rgba(255,255,255,0.04)" }}>
                <div className="flex-shrink-0 w-12 h-12 bg-green rounded-xl flex items-center justify-center">{f.icon}</div>
                <div>
                  <h3 className="font-fraunces text-[20px] lg:text-[22px] font-bold mb-1">{f.title}</h3>
      ...
      {/* ── GALLERY ── */}
      <section id="gallery" className="py-16 lg:py-24 px-4 sm:px-6 lg:px-12">
        <div className="max-w-[1280px] mx-auto">

          {/* Promo banner */}
          <div className="relative rounded-[24px] overflow-hidden flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5 sm:gap-8 mb-16 lg:mb-20 bg-green text-white px-6 py-7 sm:px-10 lg:px-12 lg:py-10">
            ...
            <div className="relative z-[2]">
              <h3 className="font-fraunces text-[22px] lg:text-[32px] font-bold mb-1">Grand Opening Special: 20% off your first detail</h3>
              <p className="opacity-90 text-[15px]">Just mention this when you book. No code, no hassle.</p>
            </div>
            <Link href="/book" className="btn btn-white relative z-[2] flex-shrink-0">Claim discount &rarr;</Link>
          </div>

          <span className="section-label">Recent Work</span>
          <h2 className="display mb-6" style={{ fontSize: "clamp(32px, 5vw, 60px)" }}>Cars we&apos;ve made <em className="italic text-green-dark font-semibold">shine.</em></h2>
          <p className="text-lg text-warm-700 max-w-2xl mb-10 lg:mb-16">A growing portfolio of local rides we&apos;ve brought back to life. New work added every week.</p>

          {/* Gallery grid */}
          <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:[grid-template-columns:repeat(4,1fr)] lg:[grid-template-rows:240px_240px]">
            {/* Featured — spans 2×2 on desktop */}
            <div className="relative rounded-[20px] overflow-hidden h-[240px] lg:h-auto cursor-pointer group lg:col-span-2 lg:row-span-2">
            ...
            {[
              ...
              <div key={item.src} className="relative rounded-[20px] overflow-hidden h-[200px] lg:h-auto cursor-pointer group">
                {/* eslint-disable-next-line @next/next/no-img-element */}
      ...
      {/* ── REVIEWS ── */}
      <section id="reviews" className="py-16 lg:py-24 px-4 sm:px-6 lg:px-12 bg-warm-100">
        <div className="max-w-[1280px] mx-auto">
          <span className="section-label">What Locals Say</span>
          <h2 className="display mb-6" style={{ fontSize: "clamp(32px, 5vw, 60px)" }}>Word travels fast<br />in a small town.</h2>
          <p className="text-lg text-warm-700 max-w-2xl mb-10 lg:mb-16">Real reviews from real neighbors. Yours will go here too once you leave one after your detail.</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      {/* ── CTA ── */}
      <section id="book" className="relative overflow-hidden bg-navy text-cream text-center py-20 lg:py-32 px-4 sm:px-6 lg:px-12">
        ...
        <h2 className="display text-cream relative mb-6" style={{ fontSize: "clamp(36px, 6vw, 80px)" }}>Ready to book?</h2>
        <p className="text-cream/80 relative max-w-xl mx-auto mb-10" style={{ fontSize: 18 }}>Pick a time that works for you. We&apos;ll call to confirm your quote based on your vehicle, and you&apos;re set.</p>
