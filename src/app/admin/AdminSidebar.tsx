    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 px-5 py-5 border-b border-white/[0.08]">
        ...
        <div>
          ...
          <div className="text-[10px] text-cream/40 uppercase tracking-[2px] mt-0.5">Admin</div>
        </div>
      </div>
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              onClick={onNav}
              className={`flex items-center gap-3 px-4 py-3 rounded-xl mb-1 no-underline transition-all duration-200 ${
                active
                  ? "bg-green text-white"
      ...
      <div className="px-3 pb-5 pt-4 border-t border-white/[0.08]">
        <button
          ...
          disabled={loggingOut}
          className="flex items-center gap-3 w-full px-4 py-3 rounded-xl text-cream/50 hover:bg-white/[0.06] hover:text-cream transition-all duration-200 text-[14px] font-semibold disabled:opacity-40"
        >
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export default function AdminSidebar() {
  ...
  return (
    <>
      ...
      {/* ── Mobile top bar ── */}
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-navy border-b border-white/[0.08] h-[60px] flex items-center px-4">
        <button
          onClick={() => setOpen((o) => !o)}
        ...
        <LogoMark />
        <span className="font-fraunces font-extrabold text-cream text-[18px] ml-2">GoHorn</span>
        <span className="text-cream/40 text-[10px] ml-2 uppercase tracking-[2px] mt-0.5">Admin</span>
      </div>
